...
namespace aDVanceERP.Core.Seguridad.Utiles {
    public static class UtilesSeguridadModulosAplicacion {
            ...
            using (var conexion = new MySqlConnection(UtilesConfServidores.ObtenerStringConfServidorMySQL())) {
                ...
                using (var transaccion = conexion.BeginTransaction()) {
                    try {
                        ...
                        long idModulo = ObtenerIdModulo(conexion, transaccion, nombreModulo);
        ...
        private static bool ExistePermiso(MySqlConnection conexion, MySqlTransaction transaccion, long idModulo, string nombrePermiso) {
            using (var comando = conexion.CreateCommand()) {
                comando.Transaction = transaccion;
                comando.CommandText = @"SELECT COUNT(*) FROM adv__permiso WHERE id_modulo = @idModulo AND nombre = @nombrePermiso;";
        ...
        private static void InsertarPermiso(MySqlConnection conexion, MySqlTransaction transaccion, long idModulo, string nombrePermiso) {
            using (var comando = conexion.CreateCommand()) {
                comando.Transaction = transaccion;
