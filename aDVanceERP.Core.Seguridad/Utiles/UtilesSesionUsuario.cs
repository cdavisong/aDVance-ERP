    ...
    public static class UtilesSesionUsuario {
        ...
        public static void IniciarSesion(int idCuentaUsuario) {
            ...
            using (var conexion = new MySqlConnection(UtilesConfServidores.ObtenerStringConfServidorMySQL())) {
                ...
                using (var comando = conexion.CreateCommand()) {
                    comando.CommandText = "SELECT * FROM adv__sesion_usuario WHERE id_cuenta_usuario = @idCuentaUsuario AND fecha_fin IS NULL";
                ...
                using (var comando = conexion.CreateCommand()) {
                    comando.CommandText = "INSERT INTO adv__sesion_usuario (id_cuenta_usuario, token, fecha_inicio) VALUES (@idCuentaUsuario, @token, @fechaInicio)";
                    comando.Parameters.AddWithValue("@idCuentaUsuario", idCuentaUsuario);
        ...
        public static void CerrarSesion(string token) {
            ...
            using (var conexion = new MySqlConnection(UtilesConfServidores.ObtenerStringConfServidorMySQL())) {
                ...
                using (var comando = conexion.CreateCommand()) {
                    comando.CommandText = "UPDATE adv__sesion_usuario SET fecha_fin = @fechaFin WHERE token = @token AND fecha_fin IS NULL";
