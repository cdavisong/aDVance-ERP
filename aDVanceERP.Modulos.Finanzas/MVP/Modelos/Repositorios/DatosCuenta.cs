...
namespace aDVanceERP.Modulos.Finanzas.MVP.Modelos.Repositorios {
    public class DatosCuenta : RepositorioDatosBase<Cuenta, CriterioBusquedaCuenta>, IRepositorioCuenta {
        public override string ComandoCantidad() {
            return "SELECT COUNT(id_cuenta) FROM adv__cuenta;";
        }

        public override string ComandoAdicionar(Cuenta objeto) {
            return $"INSERT INTO adv__cuenta (alias, numero_tarjeta, moneda, id_contacto) VALUES ('{objeto.Alias}', '{objeto.NumeroTarjeta}', {(int) objeto.Moneda}, {objeto.IdContacto});";
        }

        public override string ComandoEditar(Cuenta objeto) {
            return $"UPDATE adv__cuenta SET alias='{objeto.Alias}', numero_tarjeta='{objeto.NumeroTarjeta}', moneda={(int) objeto.Moneda}, id_contacto={objeto.IdContacto} WHERE id_cuenta={objeto.Id};";
        }

        public override string ComandoEliminar(long id) {
            return $"DELETE FROM adv__cuenta WHERE id_cuenta={id};";
        }
        ...
        public override string ComandoObtener(CriterioBusquedaCuenta criterio, string dato) {
            ...
            switch (criterio) {
                case CriterioBusquedaCuenta.Id:
                    comando = $"SELECT * FROM adv__cuenta WHERE id_cuenta={dato};";
                    break;
                case CriterioBusquedaCuenta.Alias:
                    comando = $"SELECT * FROM adv__cuenta WHERE alias LIKE '%{dato}%';";
                    break;
                default:
                    comando = "SELECT * FROM adv__cuenta;";
                    break;
            }
        ...
        public override string ComandoExiste(string dato) {
            return $"SELECT COUNT(1) FROM adv__cuenta WHERE id_cuenta = {dato};";
        }
