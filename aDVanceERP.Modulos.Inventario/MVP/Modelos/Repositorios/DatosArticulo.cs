...
public class DatosArticulo : RepositorioDatosBase<Articulo, CriterioBusquedaArticulo>, IRepositorioArticulo {
    ...
    public override string ComandoObtener(CriterioBusquedaArticulo criterio, string dato) {
        ...
        const string comandoAdicionalJoin =
            "JOIN adv__articulo_almacen ta ON t.id_articulo = ta.id_articulo JOIN adv__almacen a ON ta.id_almacen = a.id_almacen ";
        var comandoAdicionalWhere = $"AND a.nombre = '{datoMultiple[0]}'";

        switch (criterio) {
            case CriterioBusquedaArticulo.Id:
                comando =
                    $"SELECT t.*{(aplicarFiltroAlmacen ? comandoAdicionalSelect : string.Empty)} FROM adv__articulo t {(aplicarFiltroAlmacen ? comandoAdicionalJoin : string.Empty)}WHERE t.id_articulo='{(datoMultiple.Length > 0 ? datoMultiple[1] : dato)}' {(aplicarFiltroAlmacen ? comandoAdicionalWhere : string.Empty)};";
                break;
            case CriterioBusquedaArticulo.Codigo:
                comando =
                    $"SELECT t.*{(aplicarFiltroAlmacen ? comandoAdicionalSelect : string.Empty)} FROM adv__articulo t {(aplicarFiltroAlmacen ? comandoAdicionalJoin : string.Empty)}WHERE LOWER(t.codigo) LIKE LOWER('%{(datoMultiple.Length > 0 ? datoMultiple[1] : dato)}%') {(aplicarFiltroAlmacen ? comandoAdicionalWhere : string.Empty)};";
                break;
            case CriterioBusquedaArticulo.Nombre:
                comando =
                    $"SELECT t.*{(aplicarFiltroAlmacen ? comandoAdicionalSelect : string.Empty)} FROM adv__articulo t {(aplicarFiltroAlmacen ? comandoAdicionalJoin : string.Empty)}WHERE LOWER(t.nombre) LIKE LOWER('%{(datoMultiple.Length > 0 ? datoMultiple[1] : dato)}%') {(aplicarFiltroAlmacen ? comandoAdicionalWhere : string.Empty)};";
                break;
            default:
                comando =
                    $"SELECT t.*{(aplicarFiltroAlmacen ? comandoAdicionalSelect : string.Empty)} FROM adv__articulo t {(aplicarFiltroAlmacen ? comandoAdicionalJoin : string.Empty)}{(aplicarFiltroAlmacen ? comandoAdicionalWhere : string.Empty).Replace("AND", "WHERE")};";
                break;
        }
