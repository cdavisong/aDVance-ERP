                    ...
                    using (var comando = conexion.CreateCommand()) {
                        ...
                        try {
                            if (idAlmacenOrigen > 0) {
                                ...
                                comando.CommandText = "UPDATE mg__articulo_almacen SET stock = stock - @Cantidad WHERE id_articulo = @IdArticulo AND id_almacen = @IdAlmacenOrigen;";
                            ...
                            if (idAlmacenDestino > 0) {
                                ...
                                comando.CommandText = "SELECT COUNT(*) FROM mg__articulo_almacen WHERE id_articulo = @IdArticulo AND id_almacen = @IdAlmacenDestino;";
                                comando.Parameters.AddWithValue("@IdArticulo", idArticulo);
                                ...
                                if (count > 0) {
                                    ...
                                    comando.CommandText = "UPDATE mg__articulo_almacen SET stock = stock + @Cantidad WHERE id_articulo = @IdArticulo AND id_almacen = @IdAlmacenDestino;";
                                } else {
                                    // Insertar nuevo registro en la tabla `mg__articulo_almacen` para el artículo en el almacén de destino
                                    comando.CommandText = "INSERT INTO mg__articulo_almacen (id_articulo, id_almacen, stock) VALUES (@IdArticulo, @IdAlmacenDestino, @Cantidad);";
                                }
