...
namespace Manigest.Modulos.Ventas.MVP.Modelos.Repositorios {
    public class DatosPago : RepositorioDatosBase<Pago, CriterioBusquedaPago>, IRepositorioPago {
        ...
        public override string ComandoAdicionar(Pago objeto) {
            return $"INSERT INTO mg__pago (id_venta, metodo_pago, monto) VALUES ({objeto.IdVenta}, '{objeto.MetodoPago}', {objeto.Monto});";
        }

        public override string ComandoEditar(Pago objeto) {
            return $"UPDATE mg__pago SET id_venta={objeto.IdVenta}, metodo_pago='{objeto.MetodoPago}', monto={objeto.Monto} WHERE id_pago={objeto.Id};";
        }

        public override string ComandoEliminar(long id) {
            return $"DELETE FROM mg__pago WHERE id_pago={id};";
        }
        ...
        public override string ComandoObtener(CriterioBusquedaPago criterio, string dato) {
            ...
            switch (criterio) {
                case CriterioBusquedaPago.Id:
                    comando = $"SELECT * FROM mg__pago WHERE id_pago={dato};";
                    break;
                case CriterioBusquedaPago.IdVenta:
                    comando = $"SELECT * FROM mg__pago WHERE id_venta={dato};";
                    break;
                default:
                    comando = "SELECT * FROM mg__pago;";
                    break;
            }
        ...
        public override string ComandoExiste(string dato) {
            return $"SELECT COUNT(1) FROM mg__pago WHERE id_pago = {dato};";
        }
